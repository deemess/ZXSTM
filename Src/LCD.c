...
/* Private define ------------------------------------------------------------*/
/* Ê¹ÓÃ×ÜÏß·½Ê½Ê±¶¨ÒåµØÖ· */
/* ¹ÒÔÚ²»Í¬µÄBANK,Ê¹ÓÃ²»Í¬µØÖ·ÏßÊ±Çë×ÔÐÐ»»ËãµØÖ· */
...
void LCD_Initializtion(void)
{
	...
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, GPIO_PIN_SET); //Set BackLight pin on
	HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_RESET);//Reset LCD
	HAL_Delay(100);
	...
	else if( DeviceCode == 0x8989 )
	{
	    ...
	    LCD_WriteReg(0x0010,0x0000);    delay_ms(50);
			LCD_WriteReg(0x0011,0x6058);    delay_ms(50); 
...
void GUI_Image(uint16_t* imageData, uint32_t size)
{
	uint32_t p = 0;
	LCD_WriteIndex(0x22);
	for(p = 0; p<size; p++)
	{
		LCD_WriteData(imageData[p]);
	}
}
